		...
		var _tempVarsXY = {css:{}}, //speed optimization - we reuse the same vars object for x/y TweenLite.set() calls to minimize garbage collection tasks and improve performance.
			...
			Draggable = function(target, vars) {
				...
				var type = (vars.type || (_isOldIE ? "top,left" : "x,y")).toLowerCase(),
					...
					enabled, scrollProxy, startPointerX, startPointerY, startElementX, startElementY, hasBounds, hasDragCallback, maxX, minX, maxY, minY, tempVars, cssVars, touch, touchID, rotationOrigin, dirty, old, snapX, snapY, isClicking, touchEventTarget, matrix, interrupted, clickTime, startScrollTop, startScrollLeft, applyObj, allowTouchScrolling, touchDragAxis,
					...
					onPress = function(e) {
						...
						allowTouchScrolling = (!touchEventTarget || allowX === allowY) ? false : allowX ? "y" : "x";
						if (_isOldIE) {
							e = _populateIEEvent(e, true);
						} else if (!allowTouchScrolling && !self.allowEventDefault) {
							e.preventDefault();
						...
						if (allowTouchScrolling || self.autoScroll) {
					...
					onMove = function(e) {
						...
						if (_isOldIE) {
							...
							if (touchEventTarget && allowTouchScrolling && !touchDragAxis) { //Android browsers force us to decide on the first "touchmove" event if we should allow the default (scrolling) behavior or preventDefault(). Otherwise, a "touchcancel" will be fired and then no "touchmove" or "touchend" will fire during the scrolling (no good).
								touchDragAxis = (Math.abs(e.pageX + (matrix ? matrix[4] : 0) - startPointerX) > Math.abs(e.pageY + (matrix ? matrix[5] : 0) - startPointerY) && allowX) ? "x" : "y";
								...
								if (_isAndroid && allowTouchScrolling === touchDragAxis) {
									onRelease(originalEvent);
									return;
								}
							}
							if (!self.allowEventDefault && (!allowTouchScrolling || (touchDragAxis && allowTouchScrolling !== touchDragAxis)) && originalEvent.cancelable !== false) {
					...
					onRelease = function(e, force) {
						...
						if (originalEvent && !wasDragging) {
							...
							if (!_isOldIE && !self.allowEventDefault && originalEvent && (vars.dragClickables || !isClickable.call(self, originalEvent.target)) && wasDragging && (!allowTouchScrolling || (touchDragAxis && allowTouchScrolling === touchDragAxis)) && originalEvent.cancelable !== false) {
								originalEvent.preventDefault();
